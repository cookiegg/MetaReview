...
export default async function YearMdPage({
  params,
}: {
  params: { venue: string };
}) {
  const { venue,year } = await params;
  const filePath = path.join(process.cwd(), 'src/app/papers/data/', venue, year, `${venue}_${year}_summary.md`);
  ...
  try {
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="mt-2 text-gray-600">未找到会议总览</p>
      </div>
    );
  }
} 